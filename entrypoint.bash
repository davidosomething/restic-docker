...
if [ -n "${FORGET_CRON}" ]; then
  ...
  echo "${FORGET_CRON} /root/forget.bash >>/var/log/cron.log 2>&1" >>/var/spool/cron/crontabs/root
fi

if [ -n "${PRUNE_CRON}" ]; then
  __log "[INFO] Prune cron schedule is set to: ${PRUNE_CRON}"
  echo "${PRUNE_CRON} /root/prune.bash >>/var/log/cron.log 2>&1" >>/var/spool/cron/crontabs/root
fi
